...
export const ThreadPagination: FC<Props> = ({
  currentPage,
  threadsPerPage,
  totalThreads,
  showPages,
}) => {
  const pageEnd = Math.ceil(totalThreads / threadsPerPage);
      ...
      {totalThreads > threadsPerPage && (
        <Pagination>
          <PaginationContent>
            {currentPage > 1 && (
              <PaginationItem>
                <PaginationLink href={`/threads?page=${currentPage - 1}`} aria-label="前のページへ">
                  <ChevronLeft />
                </PaginationLink>
              </PaginationItem>
            )}
            ...
            {currentPage < pageEnd && (
              <PaginationItem>
                <PaginationLink href={`/threads?page=${currentPage + 1}`} aria-label="次のページへ">
                  <ChevronRight />
                </PaginationLink>
              </PaginationItem>
            )}
